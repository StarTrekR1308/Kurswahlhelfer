...
import { Search, Filter, Sparkles, AlertTriangle, CheckCircle2, ChevronRight, RotateCcw, X } from 'lucide-react';
...
export function App() {
  ...
  useEffect(() => {
    ...
    // Check if first-time user to show Tour Guide
    try {
      const tourCompleted = localStorage.getItem('bw_kurswahl_tour_v1_completed');
      if (!tourCompleted) {
        setIsTourOpen(true);
      }
    } catch (e) {
  ...
  const visibleSubjects = useMemo(() => {
    const isMandatorySubject = (fach: FachDefinition) => {
      return (
        fach.pflicht ||
        ['Deutsch', 'Mathematik', 'Geschichte', 'Geographie', 'Gemeinschaftskunde', 'Sport'].includes(fach.name) ||
        fach.attribute.includes('Fremdsprache') ||
        fach.attribute.includes('Naturwissenschaft') ||
        fach.attribute.includes('MuKu') ||
        fach.attribute.includes('ReliEthik')
      );
    };
    ...
    return FACH_LISTE.filter((fach) => {
      ...
      if (categoryFilter === 'MANDATORY') {
        return isMandatorySubject(fach);
      }
      if (categoryFilter === 'LF') {
        return belegung.some((b) => b.fachId === fach.id && b.typ === 'LF');
      }
      if (categoryFilter === 'ORAL') {
        return belegung.some((b) => b.fachId === fach.id && b.muendlich);
      }
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-3.5 mb-6 space-y-3">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            {/* Search Input with quick 'X' Clear Button */}
            <div className="relative w-full sm:w-80">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
              <input
                id="subject-search-input"
                type="text"
                placeholder="Fach suchen (z.B. Physik, Englisch, Geographie)..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-8 py-2 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              />
              {searchQuery && (
                <button
                  id="btn-clear-search"
                  onClick={() => setSearchQuery('')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-200/80 rounded-full transition-colors cursor-pointer"
            ...
            {(searchQuery.trim() !== '' || categoryFilter !== 'ALL') && (
              <div className="flex items-center gap-2 self-end sm:self-center">
                <span className="text-xs text-slate-500 font-medium">
                  {visibleSubjects.length} Treffer
                </span>
                <button
                  ...
                  title="Suche & Filter zurücksetzen"
                >
                  <X className="w-3 h-3" />
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto max-w-full pb-1 sm:pb-0 scrollbar-none pt-1 border-t border-slate-100">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider shrink-0 mr-1 hidden sm:inline">
              Filter:
            </span>
            {[
              { id: 'ALL', label: 'Alle Fächer', count: FACH_LISTE.length - excludedIds.length },
              { id: 'MANDATORY', label: 'Pflichtfächer', count: 10 },
              {
                id: 'LF',
                label: 'Leistungsfächer',
                badge: `${validation.anzahlLF}/3`,
                activeColor: 'bg-blue-600 text-white',
              },
              {
                id: 'ORAL',
                label: 'Mündl. Prüfung',
                badge: `${validation.anzahlMuendlich}/2`,
                activeColor: 'bg-indigo-600 text-white',
              },
              {
                id: 'SELECTED',
                label: 'Belegt',
                badge: `${belegung.length}`,
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer border ${
                    isActive
                      ...
                      : 'bg-slate-50 text-slate-600 border-slate-200/80 hover:text-slate-900 hover:bg-slate-100'
                  }`}
                >
                  <span>{cat.label}</span>
                  {cat.badge && (
      ...
      {/* Interactive Tour Guide Walkthrough */}
      <TourGuide
        isOpen={isTourOpen}
